...
T Config::genericGet(const char* key, T def) {
    ...
    if (inCfg->exists(key)) {
        if (!inCfg->lookupValue(key, val)) {
            throw TypeError{ "Type error on optional setting %s, expected type %s", key, getTypeName<T>() };
        }
    } else {
        val = def;
    }
...
T Config::genericGet(const char* key) {
    ...
    if (inCfg->exists(key)) {
        if (!inCfg->lookupValue(key, val)) {
            throw TypeError{ "Type error on mandatory setting %s, expected type %s", key, getTypeName<T>() };
        }
    } else {
        throw TypeError{ "Mandatory setting %s (%s) not found", key, getTypeName<T>() };
    }
    writeVar(outCfg, key, val);
    return val;
}
...
template<typename T>
T Config::genericGetNoThrow(const char* key, T def) {
    try {
        return genericGet(key, def);
    } catch (TypeError te) {
        te.handle();
    }
}
...
template<typename T>
T Config::genericGetNoThrow(const char* key) {
    try {
        return genericGet<T>(key);
    } catch (TypeError te) {
        te.handle();
    }
}
...
template<>
lc_int64 Config::genericGetNoThrow(const char* key, lc_int64 def) {
    try {
        return genericGet<int>(key);
    } catch (TypeError te) {
        try {
            return genericGet<lc_int64>(key, def);
        } catch (TypeError te2) {
            te2.handle();
        }
    }
...
template<>
lc_int64 Config::genericGetNoThrow(const char* key) {
    try {
        return genericGet<int>(key);
    } catch (TypeError te) {
        try {
            return genericGet<lc_int64>(key);
        } catch (TypeError te2) {
            te2.handle();
        }
    }
...
template<> uint32_t Config::get<uint32_t>(const char* key) {return (uint32_t) genericGetNoThrow<int>(key);}
template<> uint64_t Config::get<uint64_t>(const char* key) {return (uint64_t) genericGetNoThrow<lc_int64>(key);}
template<> bool Config::get<bool>(const char* key) {return genericGetNoThrow<bool>(key);}
template<> const char* Config::get<const char*>(const char* key) {return genericGetNoThrow<const char*>(key);}
template<> double Config::get<double>(const char* key) {return (double) genericGetNoThrow<double>(key);}

template<> uint32_t Config::get<uint32_t>(const char* key, uint32_t def) {return (uint32_t) genericGetNoThrow<int>(key, (int)def);}
template<> uint64_t Config::get<uint64_t>(const char* key, uint64_t def) {return (uint64_t) genericGetNoThrow<lc_int64>(key, (lc_int64)def);}
template<> bool Config::get<bool>(const char* key, bool def) {return genericGetNoThrow<bool>(key, def);}
template<> const char* Config::get<const char*>(const char* key, const char* def) {return genericGetNoThrow<const char*>(key, def);}
template<> double Config::get<double>(const char* key, double def) {return (double) genericGetNoThrow<double>(key, (double)def);}
