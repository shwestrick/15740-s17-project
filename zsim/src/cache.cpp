...
//        bool updateReplacement = (req.type == GETS) || (req.type == GETX);
        bool updateReplacement = true; //we would like our replacement policy to update the priorities even on a write hit
    ...
    assert_msg(respCycle >= req.cycle, "[%s] resp < req? 0x%lx type %s childState %s, respCycle %ld reqCycle %ld",
            ...
            name.c_str(), req.lineAddr, AccessTypeName(req.type), DCWSOLIStateName(*req.state), respCycle, req.cycle);
    return respCycle;
}
