...
/*        inline void monitor(bool hit, bool write) {
    ...
    public:
        AsymSRRIPReplPolicy(uint32_t _numLines, uint32_t M, uint32_t _rstart, uint32_t _wstart, uint32_t _cprom, uint32_t _dprom)
                    : SRRIPReplPolicy(_numLines, M), rstart(_rstart), wstart(_wstart), cprom(_cprom), dprom(_dprom) {
/*            rhits = 0;
            whits = 0;
            accsUntilUpdate = UPDATE_INTERVAL;*/

            info("numlines = %i, vmax = %i, rstart = %i, wstart = %i, cprom = %i, dprom = %i\n", numLines, vmax, rstart, wstart, cprom, dprom)
...
//            updateDirty(id, write);
            ...
            if (array[id] == 0) {
                ...
                bool dirty = cc->isDirty(id);//dirty_array[id];
                //This needs to check if the item is currently dirty or is being written to, not just the latter
                if (dirty){
                    changePrio(id, dprom); //predict near-immediate re-reference
                }else{
                    changePrio(id, cprom);
                }
                profPromotions.inc();
...
/*        inline void updateDirty(uint32_t id, bool write){
            dirty_array[id] = dirty_array[id] || write;
        }

        inline void clearDirty(uint32_t id){
            dirty_array[id] = false;
        }*/
