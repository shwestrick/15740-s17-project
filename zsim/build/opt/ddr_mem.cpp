...
DDRMemory::DDRMemory(uint32_t _lineSize, uint32_t _colSize, uint32_t _ranksPerChannel, uint32_t _banksPerRank,
        uint32_t _sysFreqMHz, const char* tech, const char* addrMapping, uint32_t _controllerSysLatency,
                     ...
                     bool _isNVM, uint32_t _domain, g_string& _name)
    : lineSize(_lineSize), ranksPerChannel(_ranksPerChannel), banksPerRank(_banksPerRank),
      ...
      deferredWrites(_deferredWrites), closedPage(_closedPage), isNVM(_isNVM), domain(_domain), name(_name)
{
    ...
    info("%s: domain %d, %d ranks/ch %d banks/rank, tech %s, boundLat %d rd / %d wr, %s",
         name.c_str(), domain, ranksPerChannel, banksPerRank, tech, minRdLatency, minWrLatency, isNVM? "NVM" : "DRAM");
...
void DDRMemory::initStats(AggregateStat* parentStat) {
    ...
    profRowBufCloses.init("rbCloses", "Row buffer closes"); memStats->append(&profRowBufCloses);
    profExpRowBufCloses.init("erbCloses", "Expensive row buffer closes"); memStats->append(&profExpRowBufCloses);
    ...
    switch (req.type) {
        ...
        case PUTX:
            *req.state = I;
            break;
        case GETS:
            *req.state = req.is(MemReq::NOEXCL)? S : C;
            break;
        case GETX:
            *req.state = W;
            break;
...
uint64_t DDRMemory::findMinCmdCycle(const Request& r) const {
    ...
    if (r.loc.row == bank.openRow && bank.open) {
        ...
        uint64_t prechargeLatency = (!isNVM || bank.dirty)? tRP : 0;
        uint64_t actCycle = std::max(r.arrivalCycle, std::max(preCycle + prechargeLatency, bank.lastActCycle + tRRD));
...
uint64_t DDRMemory::trySchedule(uint64_t curCycle, uint64_t sysCycle) {
    ...
    if (r->loc.row == bank.openRow && bank.open) {
        ...
        uint64_t prechargeLatency = (!isNVM || bank.dirty)? tRP : 0;
        profRowBufCloses.inc();
        if (!isNVM || bank.dirty) {
            profExpRowBufCloses.inc();
        }
        uint64_t actCycle = std::max(r->arrivalCycle, std::max(preCycle + prechargeLatency, bank.lastActCycle + tRRD));
        actCycle = std::max(actCycle, rankActWindows[r->loc.rank].minActCycle() + tFAW);
...
void DDRMemory::initTech(const char* techName) {
    ...
    if (tech.find("NVM-RW-Asymmetry-") != tech.npos) {
        size_t bw_p_loc = tech.find("_tCK");
        double tCK_mod = 3;
        if (bw_p_loc != tech.npos) {
            tCK_mod = std::stod(tech.substr(bw_p_loc+4, tech.npos));
            info("Setting DDR clock speed to %f ns", tCK_mod);
        }
        std::string asymmetryStr = tech.substr(tech.find_last_of("-")+1, bw_p_loc);
        int asymmetry;
        try {
            asymmetry = std::stoi(asymmetryStr);
        } catch (...) {
            panic("Failed to parse asymmetry: %s", asymmetryStr.c_str());
        }
        info("Initializing DDR tech with read-write asymmetry of %u", asymmetry);
        assert(asymmetry >= 1);
        
        // from DRAMSim2/ini/DDR3_micron_16M_8B_x4_sg15.ini (Micron)
        // ns; all other in mem cycles
        tCK = tCK_mod;                        // NVM slower than DDR
        tBL = 4;
        tCL = 10;
        tRCD = 10;
        tRTP = 5;
        ...
        tRFC = 74;
        tREFI = 5200;
    } else if (tech.find("DDR3-1333-CL10") != tech.npos) {
        size_t bw_p_loc = tech.find("_tCK");
        double tCK_mod = 1.5;
        if (bw_p_loc != tech.npos) {
            tCK_mod = std::stod(tech.substr(bw_p_loc+4, bw_p_loc));
            info("Setting DDR clock speed to %f ns", tCK_mod);
        }
         // from DRAMSim2/ini/DDR3_micron_16M_8B_x4_sg15.ini (Micron)
        tCK = tCK_mod;  // ns; all other in mem cycles
        tBL = 4;
        tCL = 10;
        tRCD = 10;
        tRTP = 5;
