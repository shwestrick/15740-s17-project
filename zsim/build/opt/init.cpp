...
#include "asym_rrip_repl_policies.h"
#include "asym_part_repl_policy.h"
...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    if (replType == "LRU" || replType == "LRUNoSh") {
    ...
    } else if (replType == "SRRIP" || replType == "BRRIP" || replType == "DRRIP" || replType == "TADRRIP" || replType == "ASRRIP") {
        uint32_t M = config.get<uint32_t>(prefix + "repl.M", 2);
        ...
        else if (replType == "TADRRIP"){
            bool useSetSampling = config.get<bool>(prefix + "repl.setSampling", false);
            if (useSetSampling) {
                uint32_t samplingFactor = config.get<uint32_t>(prefix + "repl.samplingFactor", 32);
                rp = new TADRRIPSetSamplingReplPolicy(numLines, M, samplingFactor);
            } else {
                rp = new TADRRIPReplPolicy(numLines, M);
            }
        } else if (replType == "ASRRIP"){
            uint32_t r_start = config.get<uint32_t>(prefix + "repl.r_start", 1); 
            uint32_t w_start = config.get<uint32_t>(prefix + "repl.w_start", 1); 
            uint32_t c_prom = config.get<uint32_t>(prefix + "repl.c_prom", 1<<M); 
            uint32_t d_prom = config.get<uint32_t>(prefix + "repl.d_prom", 1<<M); 
            rp = new AsymSRRIPReplPolicy(numLines, M, r_start, w_start, c_prom, d_prom);
        } else{
            panic("Unknown Repl Type: %s", replType.c_str());
        }
    } else if (replType == "StaticAsymmetricPart") {
        string delegateType = config.get<const char*>(prefix + "repl.delegateReplPolicy", "LRU");
        ReplPolicy* delegate;
        if (delegateType == "LRU") {
            delegate = new LRUReplPolicy<false>(numLines);
        } else if (delegateType == "SRRIP") {
            delegate = new SRRIPReplPolicy(numLines, 2);
        } else if (delegateType == "DRRIP") {
            delegate = new DRRIPSetSamplingReplPolicy(numLines, 2, 32);
        } else {
            panic("No valid delegate repl policy specified");
        }
        uint32_t dirtyWays = config.get<uint32_t>(prefix + "repl.dirtyWays", ways / 2);

        rp = new StaticAsymmetricPartReplPolicy(numLines, ways, dirtyWays, delegate);
    ...
    CC* cc;
    if (isTerminal) {
        cc = new DCWSOLITerminalCC(numLines, name);
    } else {
        cc = new DCWSOLICC(numLines, nonInclusiveHack, name);
    }
    rp->setCC(cc);
    if (!isTerminal) {
        if (type == "Simple") {
...
DDRMemory* BuildDDRMemory(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string name, const string& prefix) {
    ...
    bool isNVM = string(tech).find("NVM") != string::npos;
    ...
    auto mem = new DDRMemory(zinfo->lineSize, pageSize, ranksPerChannel, banksPerRank, frequency, tech,
                             addrMapping, controllerLatency, queueDepth, maxRowHits, deferWrites,
                             closedPage, isNVM, domain, name);
    return mem;
}
